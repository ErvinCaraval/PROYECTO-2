...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   Backend-v1 Docker Hub Push Script${NC}"
echo -e "${BLUE}========================================${NC}"
...
if [ ! -f "package.json" ]; then
    ...
    echo -e "${YELLOW}   Ejecuta este script desde la carpeta backend-v1${NC}"
    exit 1
fi
...
# Step 2: Test (opcional)
echo ""
echo -e "${BLUE}🧪 Paso 2: Probando imagen localmente (opcional)${NC}"
echo -e "${YELLOW}   Ejecutando: docker run --rm $TAG_FULL --version${NC}"
# docker run --rm "$TAG_FULL" node --version

# Step 3: Login a Docker Hub
echo ""
echo -e "${BLUE}🔐 Paso 3: Autenticándose con Docker Hub...${NC}"
...
# Step 4: Push
echo ""
echo -e "${BLUE}📤 Paso 4: Subiendo imágenes a Docker Hub...${NC}"
...
# Step 5: Verificar imágenes
echo ""
echo -e "${BLUE}🔍 Paso 5: Verificando imágenes locales...${NC}"
...
echo -e "  3. Ejecutar: ${YELLOW}docker run -p 5000:5000 $TAG_FULL${NC}"
echo ""
