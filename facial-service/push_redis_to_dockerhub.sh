...
# Push the Docker image built from the local `redis/` folder to Docker Hub.
...
IMAGE_NAME="${IMAGE_NAME:-${DOCKERHUB_USER:-ervincaravaliibarra}/facial-service-redis:latest}"
BUILD_CONTEXT_DIR="./redis"

# Docker Hub credentials (can be provided via env vars or .env)
DOCKERHUB_USER="${DOCKERHUB_USER:-ervincaravaliibarra}"
DOCKERHUB_PASS="${DOCKERHUB_PASS:-}"

echo "Push Redis image to Docker Hub"
